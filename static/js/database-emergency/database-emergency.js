...
class Room4 {
    ...
    render() {
        ...
        container.innerHTML = `
            <div class="room-container p-6 fade-in">
                ...
                <div class="database-status grid grid-cols-4 gap-3 mb-6">
                    ...
                    <div class="status-card bg-purple-900 p-3 rounded text-center">
                        <i class="bi bi-clock text-purple-400 text-xl"></i>
                        <p class="text-xs text-purple-200">Time Critical</p>
                        <p id="db-timer" class="text-lg font-bold text-purple-100">${Math.floor(this.timeRemaining / 60)}m</p>
                        <p class="text-xs text-purple-300">Remaining</p>
                    </div>
                </div>
    ...
    startDatabaseTimer() {
        this.databaseTimer = setInterval(() => {
            ...
            if (timerDisplay) {
                timerDisplay.textContent = `${Math.floor(this.timeRemaining / 60)}m`;
            }
            
            if (this.timeRemaining <= 0) {
                clearInterval(this.databaseTimer);
                this.game.gameOver('Database recovery time expired! Data corruption spread to entire system - Complete data loss.');
            }
        }, 1000);
    }
    ...
    cleanup() {
        if (this.databaseTimer) clearInterval(this.databaseTimer);
    }
}
